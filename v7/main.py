"""
Agent v7版本
在v6版本基础上增加了多Agent协作模块（消息总线 + 结构化协议）。
...
核心改动：
  ...
  多Agent协作（s15 + s16）：
    - MessageBus 文件邮箱（.mailboxes/ 目录，.jsonl 收件箱）
    - 6 个 Lead 工具：spawn_teammate / send_message / check_inbox
                      request_shutdown / request_plan / review_plan
    - Teammate idle loop + 协议消息路由（dispatch_message）
    - ProtocolState 请求-响应状态机（shutdown / plan_approval）
"""
import time
