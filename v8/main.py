"""
Agent v8版本 — 完整的 Claude Code 架构复现
在v7基础上增加了自主Agent、Worktree隔离、MCP插件系统。
整合 s17 ~ s19 的核心功能，构建出完整的 Agent 框架。

核心模块：
  system_prompt 动态组装 + 缓存
  三条错误恢复路径 + 指数退避
  任务系统（s12 + s13）：持久化 DAG + 后台异步执行
  多Agent协作（s15 + s16）：消息总线 + 结构化协议
  自主Agent（s17）：WORK/IDLE 生命周期 + 自动认领任务
  Worktree隔离（s18）：git worktree 任务绑定 + 队友 cwd 切换
  MCP插件（s19）：外部工具发现 + 动态工具池
"""
import time
...
from agent_teams import (TEAM_TOOLS, TEAM_TOOL_HANDLERS,
                         consume_lead_inbox, active_teammates)
from worktree_isolation import WORKTREE_TOOLS, WORKTREE_TOOL_HANDLERS
from mcp_plugin import (MCP_TOOLS, MCP_TOOL_HANDLERS, mcp_clients,
                         assemble_tool_pool)
...
# ── 注册多Agent协作工具（s15: 消息总线 + s16: 结构化协议）──
TOOLS.extend(TEAM_TOOLS)
TOOL_HANDLERS.update(TEAM_TOOL_HANDLERS)

# ── 注册 Worktree 隔离工具（s18）──
TOOLS.extend(WORKTREE_TOOLS)
TOOL_HANDLERS.update(WORKTREE_TOOL_HANDLERS)
...
def agent_loop(messages: list, context: dict):
    """主循环 — 动态 system prompt + 动态工具池 + 三条错误恢复路径。"""
    iteration = 0
    state = RecoveryState(primary_model=MODEL, fallback_model=FALLBACK_MODEL)
    max_tokens = MAX_TOKENS

    # s19: 动态工具池（内置 + MCP），每次 connect_mcp 后重建
    current_tools, current_handlers = assemble_tool_pool(TOOLS, TOOL_HANDLERS)
    ...
    while True:
            ...
            response = with_retry(
                lambda mt=max_tokens, mdl=state.current_model:
                    client.messages.create(
                        ...
                        tools=current_tools, max_tokens=mt),
                state)
        ...
        for block in response.content:
            ...
            handler = current_handlers.get(block.name)
            if not handler:
        ...
        # ── s19: connect_mcp 后重建工具池 + 刷新 system prompt ──
    ...
    while True:
        ...
        if inbox_msgs:
            ...
            history.append({"role": "user",
                            "content": f"[Inbox]\n{inbox_text}"})
            log_event("TEAM", "inbox_injected", count=len(inbox_msgs))
        print()
