...
class Task:
    ...
    worktree: str | None = None  # 绑定的 git worktree 名称
...
def claim_task(task_id: str, owner: str = "agent") -> str:
    """认领任务：pending → in_progress。需要依赖全部完成且无 owner。"""
    task = load_task(task_id)
...
def scan_unclaimed_tasks() -> list[dict]:
    """扫描未认领的任务：pending + 无 owner + 依赖全部完成。
    供自主 Agent 在 idle 阶段自动认领。"""
    unclaimed = []
...
# 导出给其他模块使用
__all__ = [
    "Task", "TASKS_DIR", "create_task", "save_task", "load_task",
    "list_tasks", "get_task", "can_start", "claim_task", "complete_task",
    "scan_unclaimed_tasks",
    "should_run_background", "start_background_task", "collect_background_results",
    "TASK_TOOLS", "TASK_TOOL_HANDLERS",
]
